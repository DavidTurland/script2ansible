...
if __name__ == "__main__":
    ...
    parser.add_argument("--generator", choices=["role", "role_tasks", "playbook"], default="role", help="type of thing to generate (role or playbook)")
    ...
    if args.type == "slack":
        if os.path.isdir(config["input"]):
            ...
            raise ValueError(f"Input path {args.input} is not a directory for Slack role processing.")
    elif args.type == "bash":
        if os.path.isfile(config["input"]):
            processor = BashProcessor(config["input"], config)
            processor.process()
        else:
            raise ValueError(f"Input path {args.input} is not a file for Bash processing.")
