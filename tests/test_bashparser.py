...
class TestBashParser(unittest.TestCase):
    ...
    def test_parse_basic_commands(self):
        ...
        self.assertTrue(
            any(
                t.get("ansible.builtin.file", {}).get("state") == "touch" for t in tasks
            )
        )
        self.assertTrue(
            any(
        ...
        self.assertTrue(
            any(t.get("ansible.builtin.shell") == "ldconfig" for t in tasks)
        )
        self.assertTrue(any("ansible.builtin.unarchive" in t for t in tasks))
        self.assertTrue(
            any(t.get("ansible.builtin.file", {}).get("mode") == "755" for t in tasks)
        )
        self.assertTrue(
            any("ansible.builtin.apt" in t or "ansible.builtin.yum" in t for t in tasks)
        )
        self.assertTrue(
            any(
